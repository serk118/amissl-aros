...
/* Use manual zeroing instead of memset() to avoid external symbol
 * resolution issues on AROS x86_64 ET_REL loader where the wrong
 * function can be resolved for "memset", causing a crash in h_new
 * during library initialization (illegal address access at
 * RAX=0xFFFFFFFFFFFFF490 after subtracting 0xb70 from a NULL ptr).
 * NB: volatile is required to prevent GCC -O3 from pattern-recognizing
 * the loop and re-emitting a memset() call. */
static void memzero(void *ptr, unsigned long len)
{
...
struct HashTable *h_new(long InitialSize,void STDARGS *(*allocator)(long),void STDARGS (*deallocator)(void *))
{
	...
	if( (new = allocator(sizeof(struct HashTable))) )
	{
		...
		{
			memzero(new->HashEntries,sizeof(struct HashEntry)*InitialSize);
			new->Locked=0;
			new->Entries=0;
...
static void *grow(struct HashTable *h)
{
	...
	if( (h->HashEntries = h->allocator(sizeof(struct HashEntry)*((h->TableSize+1)*2-1))) )
	{
		...
		memzero(h->HashEntries,sizeof(struct HashEntry)*h->TableSize);
