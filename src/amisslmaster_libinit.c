...
#if defined(__AROS__)
#include "amisslmaster_stubs_aros.h"
#endif
...
struct LibraryHeader * LIBFUNC LibOpen(REG(d0, UNUSED ULONG version), REG(a6, struct LibraryHeader *base))
{
  ...
  if(newLib != NULL)
  {
    ...
    CopyMem((APTR)((IPTR)base - (IPTR)base->libBase.lib_NegSize), newLib, base->libBase.lib_NegSize + base->libBase.lib_PosSize);

    // Set child library base
    child = (APTR)((IPTR)newLib + (IPTR)base->libBase.lib_NegSize);
  }
  #else
  ...
  if(child != NULL)
  {
    ...
    #elif defined(__AROS__)
    // Copy data segment if present (AROS uses PIC so no relocation needed)
    if(base->dataSize)
    {
      APTR datamem = (APTR)(( (IPTR)newLib + (IPTR)base->libBase.lib_NegSize + (IPTR)base->libBase.lib_PosSize + 15) & ~(IPTR)15);
      CopyMem(base->dataSeg, datamem, base->dataSize);
      child->dataSeg = datamem;
    }
...
BPTR LIBFUNC LibClose(REG(a6, struct LibraryHeader *base))
{
  ...
  if(base->libBase.lib_OpenCnt == 0)
  {
    ...
    #elif defined(__MORPHOS__) || defined(__AROS__)
    LIB___UserLibCleanup((__BASE_OR_IFACE_TYPE)base);
    FreeVec((APTR)((IPTR)(base) - (IPTR)(base->libBase.lib_NegSize)));
    #else
      #error PLATFORM not supported
    #endif
