...
#if defined(__GNUC__) && (defined(__amigaos3__) || defined(__AROS__))
#define FORCED_CONST const __attribute__ ((section (".text")))
#endif

#if defined(__AROS__)
#define ROMTAG_SCOPE /* global */
#endif
...
ROMTAG_SCOPE FORCED_CONST USED_VAR struct Resident ROMTag =
{
  RTC_MATCHWORD,
  (struct Resident *)&ROMTag,
  (struct Resident *)((IPTR)&ROMTag + 48),
    ...
    #endif
    {
	 ...
	 && (base->parent = InitMultiBase(base))
	 ...
	 )
      {
        ...
        __MathIeeeDoubTransBase = MathIeeeDoubTransBase = OpenLibrary("mathieeedoubtrans.library", 37);
        __ctor_math_init();
        #endif
