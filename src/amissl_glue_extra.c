...
// --- Direct SSL connection helper (bypasses LVO dispatch) ---

#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>

/* Connect to host:port and perform TLS handshake, returning SSL*.
 * Uses internal OpenSSL calls (no AmiSSL LVO dispatch) so it works
 * correctly on AROS x86_64. Returns NULL on failure.
 * The caller must SSL_free() the returned pointer. */
SSL *AmiSSL_direct_connect(SSL_CTX *ctx, const char *host, int port)
{
    SSL *ssl;
    BIO *bio;
    int sock;
    ...
    /* Resolve hostname. We use the stack (via bsdsocket.library or syscalls) */
    /* Create a connect BIO to handle the TCP connection */
    bio = BIO_new_connect(host);
    if (bio == NULL) {
        SSL_free(ssl);
        return NULL;
    }
    /* Set port: host string needs ":port" appended */
    /* BIO_new_connect parses "hostname:port" format */

    /* Set SNI */
    SSL_set_tlsext_host_name(ssl, host);

    /* Set fd on SSL object via BIO */
    /* Actually, let's use the simpler approach: BIO chain */
