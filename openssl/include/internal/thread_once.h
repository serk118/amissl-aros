 ...
 */
#ifdef __AROS__
/* On AROS, RUN_ONCE uses a direct inline expansion instead of calling
 * CRYPTO_THREAD_run_once (which goes through PLT32 and hangs on AROS x86_64).
 * This is safe because AmiSSL is single-threaded. */
#define RUN_ONCE(once, init) \
    ((*(once) == CRYPTO_ONCE_STATIC_INIT) ? \
     (*(once) = 1, init##_ossl_(), init##_ossl_ret_) : \
     init##_ossl_ret_)

#define RUN_ONCE_ALT(once, initalt, init) \
    ((*(once) == CRYPTO_ONCE_STATIC_INIT) ? \
     (*(once) = 1, initalt##_ossl_(), init##_ossl_ret_) : \
     init##_ossl_ret_)
#else
#define RUN_ONCE(once, init) \
