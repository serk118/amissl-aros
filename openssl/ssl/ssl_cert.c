...
CERT *ssl_cert_dup(CERT *cert)
{
    ...
    DBG_PV("=== ssl_cert_dup OPENSSL_zalloc=",(long)ret);
    if (ret == NULL) {
        DBG_P("=== ssl_cert_dup zalloc FAILED ===\n");
        return NULL;
    }

    ret->ssl_pkey_num = cert->ssl_pkey_num;
    ret->pkeys = OPENSSL_calloc(ret->ssl_pkey_num, sizeof(CERT_PKEY));
    DBG_PV("=== ssl_cert_dup calloc=",(long)ret->pkeys);
    if (ret->pkeys == NULL) {
        DBG_P("=== ssl_cert_dup calloc FAILED ===\n");
        OPENSSL_free(ret);
        return NULL;
    }

    DBG_P("=== ssl_cert_dup CRYPTO_NEW_REF ===\n");
    ret->key = &ret->pkeys[cert->key - cert->pkeys];
    if (!CRYPTO_NEW_REF(&ret->references, 1)) {
        DBG_P("=== ssl_cert_dup NEW_REF FAILED ===\n");
        OPENSSL_free(ret->pkeys);
        OPENSSL_free(ret);
        return NULL;
    }
    ...
    DBG_PV("=== ssl_cert_dup loop ssl_pkey_num=", ret->ssl_pkey_num);
...
#endif
    DBG_P("=== ssl_cert_dup SUCCESS ===\n");
    return ret;

 err:
    DBG_P("=== ssl_cert_dup ERROR ===\n");
    ssl_cert_free(ret);
