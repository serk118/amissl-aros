    ...
    const SSL_METHOD *method)
{
    ...
    DBG_P("=== ossl_ssl_connection_new_int START ===\n");
    ...
    if (!ossl_ssl_init(ssl, ctx, method, SSL_TYPE_SSL_CONNECTION)) {
        DBG_P("=== ossl_ssl_init FAILED ===\n");
        OPENSSL_free(s);
        s = NULL;
        ssl = NULL;
        goto sslerr;
    }
    DBG_P("=== ossl_ssl_init OK ===\n");

    DBG_P("=== RECORD_LAYER_init ===\n");
    RECORD_LAYER_init(&s->rlayer, s);
    DBG_P("=== RECORD_LAYER_init OK ===\n");
    ...
    DBG_P("=== sk_SSL_CIPHER_dup ===\n");
    s->tls13_ciphersuites = sk_SSL_CIPHER_dup(ctx->tls13_ciphersuites);
    DBG_PV("=== sk_SSL_CIPHER_dup result=", (long)s->tls13_ciphersuites);
    if (s->tls13_ciphersuites == NULL)
        goto cerr;

    DBG_P("=== ssl_cert_dup ===\n");
    s->cert = ssl_cert_dup(ctx->cert);
    DBG_PV("=== ssl_cert_dup result=", (long)s->cert);
    if (s->cert == NULL) {
        DBG_P("=== ssl_cert_dup FAILED ===\n");
        goto sslerr;
    }
    DBG_P("=== ssl_cert_dup OK ===\n");

    DBG_P("=== record_layer setup ===\n");
    ...
    DBG_P("=== SSL_CTX_up_ref ===\n");
    if (!SSL_CTX_up_ref(ctx)) {
        DBG_P("=== SSL_CTX_up_ref FAILED ===\n");
        goto err;
    }
    DBG_P("=== SSL_CTX_up_ref OK ===\n");
    ...
    DBG_P("=== method->ssl_init ===\n");
    if (!method->ssl_init(ssl)) {
        DBG_P("=== method->ssl_init FAILED ===\n");
        goto sslerr;
    }
    DBG_P("=== method->ssl_init OK ===\n");

    s->server = (method->ssl_accept == ssl_undefined_function) ? 0 : 1;

    DBG_P("=== method->ssl_reset ===\n");
    if (!method->ssl_reset(ssl)) {
        DBG_P("=== method->ssl_reset FAILED ===\n");
        goto sslerr;
    }
    DBG_P("=== method->ssl_reset OK ===\n");
    ...
    DBG_P("=== ossl_ssl_connection_new_int SUCCESS ===\n");
    return ssl;
cerr:
    ERR_raise(ERR_LIB_SSL, ERR_R_CRYPTO_LIB);
    goto err;
...
int SSL_do_handshake(SSL *s)
{
    ...
    DBG_PV("=== hf=", (long)sc->handshake_func);

    DBG_P("=== check_finish_init ===\n");
    if (!ossl_statem_check_finish_init(sc, -1))
        return -1;
    DBG_P("=== check_finish_init OK ===\n");

    DBG_P("=== reneg_check ===\n");
    s->method->ssl_renegotiate_check(s, 0);
    DBG_P("=== reneg_check OK ===\n");
    ...
    const char *properties)
{
     ...
     */
    ERR_set_mark();
    ciph = EVP_get_cipherbyname(OBJ_nid2sn(nid));
    if (ciph != NULL) {
        OSSL_PARAM params[2];
        int decrypt_only = 0;
