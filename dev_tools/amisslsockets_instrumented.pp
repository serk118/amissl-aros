...
// temporary debug: append a line to T:sl.log (falta fsync, flushed each call)
var
  DbgF: TextFile;
  DbgOpened: Boolean;

procedure SockDbg(Line: string);
begin
  try
    if not DbgOpened then
    begin
      AssignFile(DbgF, 'SYS:sl.log');
      Rewrite(DbgF);
      DbgOpened := True;
    end
    else
...
begin
  ...
  SockDbg('Connect enter');
  Result := InitContext(False);
  // try again
  if Not Result then
  begin
    Sleep(100);
    SockDbg('Connect retry InitContext');
    Result := InitContext(False);
  end;
  if Result then
  begin
    ...
    begin
      SockDbg('Connect step A: DO_STATE_MACHINE (pre-SET_CONNECT)');
      ...
      SockDbg('Connect step B: SET_CONNECT done');
    end;
   BIO_ctrl(web, BIO_C_DO_STATE_MACHINE,0, nil);
   SockDbg('Connect step C: DO_STATE_MACHINE returned');
   if Result then
     Result:= DoVerifyCert;
   if Result then
     SetSSLActive(True);
   SockDbg('Connect done Result=' + BoolToStr(Result, True));
   end;
end;
...
begin
  ...
  FSSLLastErrorString:='';
  Result := -1;
  SockDbg('Send enter Count=' + IntToStr(Count));
  if Assigned(Web) then
  begin
    Result := BIO_write(web, @Buffer, Count);
    SockDbg('Send BIO_write ret=' + IntToStr(Result));
    if Result < 0 then
      FetchErrorInfo;
  end;
  {repeat
...
begin
  ...
  FSSLLastError:=0;
  FSSLLastErrorString:= '';
  Result := 0;   // default EOF
  SockDbg('Recv enter Count=' + IntToStr(Count));
  if Assigned(Web) then
  begin
    { loop across SSL WANT_READ/WANT_WRITE so a partially-read/finished
      chunked response does not block caller forever on the next read }
    Result := BIO_read(web, @(Buffer), Count);
    SockDbg('Recv BIO_read ret=' + IntToStr(Result));
    if Result > 0 then
    begin
      // dump received bytes as readable text to see if response is complete
      SetLength(Rdump, Result);
      Move(Buffer, Rdump[1], Result);
      SockDbg('Recv data>>>' + Rdump + '<<<');
    end;
    if Result < 0 then
    begin
      SockDbg('Recv ret<0 -> EOF(0) to unblock caller');
      Result := 0; // signal EOF / no-data to caller so it does not block again
    end
    else if Result = 0 then
      SockDbg('Recv BIO_read EOF (0)');
  end;
...
begin
  ...
  SockDbg('BytesAvailable=' + IntToStr(Result));
end;
...
initialization
  SockDbg('INIT: enter, AmiSSLBase=' + IntToStr(LongInt(PtrUInt(AmiSSLBase))));
  if Assigned(AmiSSLBase) then
    TSSLSocketHandler.SetDefaultHandlerClass(TAmiSSLSocketHandler);
  SockDbg('INIT: default handler registered');
