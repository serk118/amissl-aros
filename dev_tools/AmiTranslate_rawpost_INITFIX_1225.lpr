...
var
  RawSSLReady: Boolean = False;
  RawSSLErrNo: Integer = 0;
...
type
  ...
  private
    ...
    function RawPost(URL, Form: string): AnsiString;
...
begin
  ...
  //
    RawStr := RawPost(BaseURL, FormText);
...
function TMainWindow.RawPost(URL, Form: string): AnsiString;
var
  ctx: PSSL_CTX;
  web: PBIO;
...
begin
  Result := '';
  // The raw-BIO path bypasses the old fphttpclient/socket-handler flow which
  // was the only place InitAmiSSLA got called. Without it the library's
  // per-task state is never created and the SSL calls take a NULL state
  // (crash in OpenTimer on AROS). Init once per program.
  if not RawSSLReady then
  begin
    if InitAmiSSL([
      AmiSSL_ErrNoPtr, AsTag(@RawSSLErrNo),
  ...
  host := URL;
  if Pos('https://', host) = 1 then Delete(host, 1, 8);
  path := '/';
  r := Pos('/', host);
  if r > 0 then
  begin
    path := Copy(host, r, MaxInt);
    host := Copy(host, 1, r - 1);
  end;
  ...
  if r < 0 then begin BIO_free_all(web); SSL_CTX_free(ctx); Exit; end;
  guard := 0;
  repeat
