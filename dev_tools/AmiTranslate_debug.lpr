...
begin
  ...
  try
    ...
    // DEBUG: write raw response + state to SYS:translate_dbg.txt
    AssignFile(DbgF, 'SYS:translate_dbg.txt');
    Rewrite(DbgF);
    try
      WriteLn(DbgF, 'BaseURL=', BaseURL);
      WriteLn(DbgF, 'FormText=', FormText);
      Try
        RawStr := hp.FormPost(BaseURL, FormText);
        WriteLn(DbgF, 'FormPost OK.');
      except
        on E: Exception do
        begin
          WriteLn(DbgF, 'FormPost EXCEPT: ' + E.Classname + ': ' + E.Message);
          CloseFile(DbgF);
          raise;
        end;
      end;
      WriteLn(DbgF, 'RawStrLen=', Length(RawStr));
      WriteLn(DbgF, 'RawStr_Begin>>>' + Copy(RawStr,1,300));
      WriteLn(DbgF, 'RawStr_End<<<' + Copy(RawStr, Length(RawStr)-200, 200));

      Try
        JData := GetJSON(RawStr, False);
        JObject := JData as TJSONObject;
        JData := JObject.FindPath('translations[0]');
        WriteLn(DbgF, 'GetJSON/FindPath OK, class=', JData.ClassName,
                ' isTJSONObject=', JData is TJSONObject);
      except
        on E: Exception do
        begin
          WriteLn(DbgF, 'GetJSON EXCEPT: ' + E.Classname + ': ' + E.Message);
          CloseFile(DbgF);
          raise;
        end;
      end;
      ...
      on E: Exception do
      begin
        WriteLn(DbgF, 'outer EXCEPT: ' + E.Classname + ': ' + E.Message);
        CloseFile(DbgF);
        raise;
      end;
    end;
    ...
    begin
      ...
      try WriteLn(DbgF, 'TRANS OK: ' + TransText); except end;
    ...
    begin
      try WriteLn(DbgF, 'JData NOT a TJSONObject (grey-button branch)'); except end;
      ShowMessage('Answer:' + RawStr);
      SL.Clear;
    end;
    try CloseFile(DbgF); except end;
  finally
    hp.Free;
  end;
