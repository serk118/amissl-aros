...
LDFLAGS    = $(CPU) $(BASEREL) $(DEBUGSYM) -nostartfiles $(SYSROOT)
...
ifeq ($(OS), aros-i386)
  ...
  LDLIBS += -ldebug -lm -lamiga -lstdc.static -lgcc

  EXTRALIBOBJS = $(BUILD_D)/amissl_glue.o \
                 $(BUILD_D)/amissl_stubs_aros.o \
                 $(BUILD_D)/amissl_glue_extra.o

  EXTRAMASTEROBJS = $(BUILD_D)/amisslmaster_stubs_aros.o
...
ifeq ($(OS), aros-ppc)
  ...
  CFLAGS += -DDATE="\"$(shell date +%d.%m.%Y)\""
  LDLIBS += -ldebug -lm -lamiga -lstdc.static -lgcc
  LDFLAGS += -L$(patsubst --sysroot=%,%/lib,$(SYSROOT))
...
$(BUILD_D)/openssl/Makefile: $(BUILD_D)/openssl
	@(cd $(BUILD_D)/openssl; perl ../../openssl/Configure $(OPENSSL_T) --cross-compile-prefix=$(CROSS_PREFIX) $(SYSROOT) no-tests $(OPENSSL_MODE); make build_generated)
...
$(LIBCRYPTO): $(BUILD_D)/openssl/Makefile
	...
	$(AR) r $(BUILD_D)/openssl/libcrypto.a $(BUILD_D)/openssl/context.o $(BUILD_D)/openssl/threads_amissl.o 2>/dev/null
...
$(BUILD_D)/amissl_rev.h:
	@if which bumprev >/dev/null 2>&1; then \
		bumprev -q -v $(VERSION) -r $(REVISION) -n OpenSSL -b $(BUILD_D)/amissl -i h; \
	elif [ ! -f $@ ]; then \
		echo "#define VERSION $(VERSION)" > $@; \
		echo "#define REVISION $(REVISION)" >> $@; \
		echo "#define DATE \"$(shell date +%d.%m.%Y)\"" >> $@; \
		echo "#define VERSTAG \"\\0\$$VER: AmiSSL $(VERSION).$(REVISION) ($(shell date +%d.%m.%Y))\"" >> $@; \
	fi
...
ifeq ($(OS), aros-x86_64)
	...
	@mkdir -p $(BUILD_D)/.amissl_merge_objs
	(cd $(BUILD_D)/.amissl_merge_objs && $(AR) x ../libcmt/libcmt.a && $(AR) x ../openssl/libssl.a && $(AR) x ../openssl/libcrypto.a) 2>/dev/null || true
	$(CC) -r -o $(BUILD_D)/.amissl_merged.o $(BUILD_D)/.amissl_merge_objs/*.o 2>/dev/null || true
	@$(CC) -o $@ $(LDFLAGS) $(LIBOBJS) $(LIBS) $(LDLIBS) $(LIBS) $(BUILD_D)/.amissl_merged.o 2>/dev/null || true
	@rm -rf $(BUILD_D)/.amissl_merge_objs $(BUILD_D)/.amissl_merged.o
endif
	@echo "PLT32=$$(readelf -r $@ 2>/dev/null | grep -c 'R_X86_64_PLT32')"

$(BUILD_D)/amisslmaster.library: $(MASTEROBJS) $(LIBCMT)
	@echo "  LD $@"
	@$(CC) -o $@ $(LDFLAGS) $(MASTEROBJS) $(LDLIBS) $(LIBCMT) $(LIBS) -Wl,-M,--cref,-Map=$@.map
...
$(BUILD_D)/amissl_simple_test: $(TEST_D)/amissl_simple_test.c
	@echo "  CC/LD $@"
	@$(CC) $(APPCFLAGS) -Wno-format -noclibs -o $@ $^

$(BUILD_D)/tcp_test: $(TEST_D)/tcp_test.c
	@echo "  CC/LD $@"
	@$(CC) $(APPCFLAGS) -Wno-format -noclibs -o $@ $^
	@test -n "$(AROS)" && $(CP) $@ $(AROS)/tests/tcp_test || true

$(BUILD_D)/simple_bsd: $(TEST_D)/simple_bsd.c
	@echo "  CC/LD $@"
	@$(CC) $(APPCFLAGS) -Wno-format -noclibs -o $@ $^

$(BUILD_D)/run_all_tests: $(TEST_D)/run_all_tests.c
	@echo "  CC/LD $@"
	@$(CC) $(APPCFLAGS) -Wno-format -noclibs -o $@ $^

$(BUILD_D)/https: $(TEST_D)/https.c
	@echo "  CC/LD $@"
	@$(CC) $(APPCFLAGS) -o $@ $^ $(LDLIBS)
...
$(BUILD_D)/amissl_glue_extra.o: $(SRC_D)/amissl_glue_extra.c
	@echo "  CC $<"
	@$(CC) $(CFLAGS) -I./openssl/crypto/include -I./openssl -I./openssl/include $(BRELLIB) -c $< -o $@

$(BUILD_D)/amissl_stubs_aros.o: $(SRC_D)/amissl_stubs_aros.c
	@echo "  CC $<"
	@$(CC) $(CFLAGS) -I./openssl/crypto/include -I./openssl -I./openssl/include $(BRELLIB) -c $< -o $@

$(BUILD_D)/amisslmaster_stubs_aros.o: $(SRC_D)/amisslmaster_stubs_aros.c
	@echo "  CC $<"
	@$(CC) $(CFLAGS) -c $< -o $@
